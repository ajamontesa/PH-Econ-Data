...
# Load Agriculture and Food Data ------------------------------------------
# Source loadAgriData.R, which builds the tidy agriculture tables from the
# committed OpenStat CSVs (downloaded by downloadAgriData.R). For a general
# dashboard we keep the annual headline tables plus national monthly prices;
# the finer quarterly splits (crops_quarterly, livestock_quarterly,
# fisheries_quarterly, prices_annual) and the province-level price detail stay
# available in loadAgriData.R for deep dives but are not folded in here.

source("Scripts/loadAgriData.R")

prices_monthly <- prices_monthly %>%      # national-level food prices only
    filter(Level == "National")



# Load Trade Data ---------------------------------------------------------
# Read the compact trade bundle built by loadTradeData.R. tradeData.rds is
# committed; the ~2.8 GB of raw per-year IMTS CSVs (Data/Trade/Exports and
# Data/Trade/Imports) are gitignored and regenerated with downloadTradeData.R.
# Keep the headline who/what tables; the partner x group and minerals deep-dive
# tables stay in the bundle but are not folded in here.

tradeData <- readRDS("Data/Trade/tradeData.rds")

trade_annual         <- tradeData$trade_annual
trade_by_partner     <- tradeData$trade_by_partner
trade_by_region      <- tradeData$trade_by_region
trade_by_commodity   <- tradeData$trade_by_commodity
trade_semiconductors <- tradeData$trade_semiconductors



# Save the dashboard data -------------------------------------------------
# Explicit save() (not save.image) so dashboardData.RData holds only the
# dashboard-relevant tables -- scratch vectors (snaColNames81/00, sna_ncols,
# main_commodities) and intermediate fiscal frames (ngdebtservice,
# ngdebt_quarterly, ngdebt_annual; ngdebt already combines them) are left out.

save(
    ## National Accounts
    SNA2000_Quarterly_Exp, SNA2000_Quarterly_Ind, SNA2000_Quarterly_PC,
    ## CPI and Inflation
    inflation_small, inflation_main, inflation_core, deflators,
    ## Labor and Employment
    labor,
    ## Fiscal
    ngcor, ngdebt, tax_types,
    ## Agriculture and Food
    crops_annual, livestock_annual, fisheries_annual,
    value_production_quarterly, self_sufficiency_annual, prices_monthly,
    ## Trade
    trade_annual, trade_by_partner, trade_by_region, trade_by_commodity,
    trade_semiconductors,
    file = "Data/dashboardData.RData"
)
